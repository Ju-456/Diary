...
void LogInUser(User *pUser, char *FolderName, Page **page1,int *nb_pages, int PageToDelete);
void CreateAnUser(User *pUser, char *FolderName, Page **page1,int *nb_pages, int PageToDelete);
...
void DataUser(User *pUser, char *FolderName, Page **page1,int *nb_pages, int PageToDelete) {
    ...
    if (strcmp(snprintf(FolderName, 1024, "/home/ju456/%s", pUser->UIdentifiant))==0){
        printf("You already have an account !\n Please Log in here :");
        LogInUser(pUser, FolderName, page1, nb_pages, PageToDelete); // si compte identification
    }else {
        printf("This identifiant doesn't exist.. Do you want to create an account ? (yes/no)");
        char reponse1[4];
        scanf("%3s", reponse1);
            if (strcmp(reponse1, "yes") == 0){
                printf("Go to create an account !");
                CreateAnUser(pUser, FolderName, page1, nb_pages, PageToDelete); // Si pas de compte, créer un compte 
            } else {
                ByeJournal();
            }
    }
...
void LogInUser(User *pUser, char *FolderName, Page **page1,int *nb_pages, int PageToDelete){
    ...
    if (strcmp(snprintf(FolderName, 1024, "/home/ju456/%s_%s", pUser->UIdentifiant, pUser->UPassword))==0){
        ...
        if (TooLate < 3){
            ...
            LogInUser(pUser, FolderName, page1, nb_pages, PageToDelete);
        } else {
...
void CreateAnUser(User *pUser, char *FolderName, Page **page1,int *nb_pages, int PageToDelete){
    ...
    if (strcmp(pUser->UPassword, UPassword2) == 0) {
        ...
        CreateAnUser(pUser, FolderName, page1, nb_pages, PageToDelete);
    }
}
